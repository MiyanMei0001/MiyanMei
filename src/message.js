...
async function GroupUpdate(Miyan, update) {
	try {
		for (let n of update) {
			...
			if (setinfo) {
				let profile;
				try {
					profile = await Miyan.profilePictureUrl(n.id, 'image');
				} catch {
					profile = 'https://telegra.ph/file/95670d63378f7f4210f03.png';
				}
				if (n.announce) {
					await Miyan.sendMessage(n.id, {
...
async function GroupParticipantsUpdate(Miyan, { id, participants, action }) {
	try {
		let welcome = global.db.groups[id].welcome
		if (welcome) {
			let metadata = await Miyan.groupMetadata(id);
			for (let n of participants) {
				...
				MiyanWlcm = await getBuffer(profile)
                MiyanLft = await getBuffer(profile)
				if (action == 'add') {
				let MiyanName = n
                const xdate = moment.tz('Jakarta').locale('en-IN').format('DD/MM/YYYY');
                const xtime = moment().tz('Jakarta').locale('en-IN').format('HH:mm:ss');
  ...
  viewOnceMessage: {
    message: {
        ...
        interactiveMessage: proto.Message.InteractiveMessage.create({
          ...
          ...await prepareWAMessageMedia({ image: MiyanWlcm }, { upload: Miyan.waUploadToServer })
          }),
...
await Miyan.relayMessage(id, msgs.message, {})
				} else if (action == 'remove') {
				let MiyanName = n
                const Miyandate = moment.tz('Jakarta').locale('en-IN').format('DD/MM/YYYY');
                const Miyantime = moment().tz('Jakarta').locale('en-IN').format('HH:mm:ss');
	            const Miyanmembers = metadata.participants.length
					Miyanbody = `
┌─❖
│『  *Gᴏᴏᴅʙʏᴇ..!! 🍁*  』 
└┬
  ...
  viewOnceMessage: {
    message: {
        ...
        interactiveMessage: proto.Message.InteractiveMessage.create({
          ...
          header: proto.Message.InteractiveMessage.Header.create({
          hasMediaAttachment: false,
          ...await prepareWAMessageMedia({ image: MiyanLft }, { upload: Miyan.waUploadToServer })
          }),
          nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
            buttons: [{
            "name": "quick_reply",
...
const Miyandate = moment.tz('Jakarta').locale('en-IN').format('DD/MM/YYYY');
let MiyanName = n
Miyanbody = ` 𝗖𝗼𝗻𝗴𝗿𝗮𝘁🎉 @${MiyanName.split("@")[0]}, you have been *promoted* to *admin* 🥳`
...
const Miyandate = moment.tz('Jakarta').locale('en-IN').format('DD/MM/YYYY');
let MiyanName = n
Miyanbody = `𝗢𝗼𝗽𝘀‼️ @${MiyanName.split("@")[0]}, you have been *demoted* from *admin* 😬`
await Miyan.sendMessage(id,
 { text: Miyanbody,
 contextInfo:{
 mentionedJid:[n],
...
async function MessagesUpsert(Miyan, message, store) {
	try {
		let botNumber = await Miyan.decodeJid(Miyan.user.id);
		const msg = message.messages[0];
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		if (!Miyan.public && !msg.key.fromMe && message.type === 'notify') return
		if (msg.key.id.startsWith('BAE5')) return
		if (msg.key.id.length === 22) return
		if (!msg.message) return
		const m = await Serialize(Miyan, msg, store)
		require('../MiyanV4')(Miyan, m, message, store);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			...
			Miyan.ev.emit('messages.upsert', pbr);
		}
		let antiswview = global.db.settings[botNumber].antiswview
		if (antiswview) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await Miyan.readMessages([msg.key]);
				// if (/protocolMessage/i.test(type)) Miyan.sendFromOwner(ownernumber, 'Status from @' + msg.key.participant.split('@')[0] + ' Was removed', msg, { mentions: [msg.key.participant] });
				// if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					// await Miyan.sendFromOwner(ownernumber, `Story from @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				// }
			}
		}
	} catch (e) {
...
async function Solving(Miyan, store) {
	...
	Miyan.getName = (jid, withoutContact  = false) => {
		...
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || Miyan.groupMetadata(id) || {};
	...
	Miyan.copyNForward = async (jid, message, forceForward = false, options = {}) => {
let vtype
...
await Miyan.relayMessage(jid, waMessage.message, { messageId:  waMessage.key.id })
return waMessage
}
	...
	Miyan.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
    }
	...
	Miyan.sendPoll = (jid, name = '', values = [], selectableCount = 1) => {
		return Miyan.sendMessage(jid, { poll: { name, values, selectableCount }})
	}
	
	Miyan.sendText = (jid, text, quoted = '', options) => Miyan.sendMessage(jid, {
        text: text,
        ...options
    }, {
        quoted,
	...
	Miyan.sendAudio = async (jid, path, quoted = '', ptt = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await Miyan.sendMessage(jid, { audio: buffer, ptt: ptt, ...options }, { quoted })
    }
    
    Miyan.sendVideo = async (jid, path, caption = '', quoted = '', gif = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await Miyan.sendMessage(jid, { video: buffer, caption: caption, gifPlayback: gif, ...options }, { quoted })
    }

Miyan.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
...
await Miyan.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
return buffer
}
Miyan.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
let buffer
if (options && (options.packname || options.author)) {
...
Miyan.sendImage = async (jid, path, caption = '', quoted = '', options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,` [1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await Miyan.sendMessage(jid, {
            image: buffer,
            caption: caption,
            ...options
        }, {
	...
	Miyan.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		if (mime.includes('gif')) {
			return Miyan.sendMessage(jid, { video: buffer.data, caption: caption, gifPlayback: true, ...options }, { quoted });
		} else if (mime === 'application/pdf') {
			return Miyan.sendMessage(jid, { document: buffer.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
		} else if (mime.includes('image')) {
			return Miyan.sendMessage(jid, { image: buffer.data, caption: caption, ...options }, { quoted });
		} else if (mime.includes('video')) {
			return Miyan.sendMessage(jid, { video: buffer.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
		} else if (mime.includes('audio')) {
			return Miyan.sendMessage(jid, { audio: buffer.data, mimetype: 'audio/mpeg', ...options }, { quoted });
		}
	}
	
	Miyan.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
  let type = await Miyan.getFile(path, true);
  ...
  try {
    m = await Miyan.sendMessage(jid, message, { ...opt, ...options });
  } catch (e) {
    //console.error(e)
    m = null;
  } finally {
    if (!m) m = await Miyan.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options });
    file = null;
    return m;
  }
}
	...
	Miyan.sendFakeLink = async (jid, text, title, body, thumbnail, myweb, options = {}) => {
		await Miyan.sendMessage(jid, {
			text: text,
			contextInfo: {
				externalAdReply: {
					title: title,
	...
	Miyan.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await Miyan.sendMessage(a + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	Miyan.parseMention = (text = '') => {
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
}
	
	Miyan.sendTextMentions = async (jid, text, quoted, options = {}) => Miyan.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	Miyan.sendAsSticker = async (jid, path, quoted, type, options = {}) => {
		let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0);
		if (options && (options.packname || options.author)) {
		...
		await Miyan.sendMessage(jid, { sticker: { url: buff }, ...options }, { quoted });
		return buff;
	}
...
Miyan.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
	...
	Miyan.getFile = async (PATH, save) => {
		...
		filename = path.join(__filename, '../MiyanMedia/trash/' + new Date * 1 + '.' + type.ext)
	...
	Miyan.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		...
		await Miyan.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
		return fs.promises.unlink(pathFile);
	}
	
	 Miyan.sendButtonImage = async (jid, buttons, quoted, opts = {}) => {
      ...
      return Miyan.relayMessage(jid, message["message"], {
         messageId: message.key.id
      })
   }
   
   Miyan.sendButtonVideo = async (jid, buttons, quoted, opts = {}) => {
   ...
   Miyan.sendList = async (jid, sections, quoted, opts = {}) => {
      let message = generateWAMessageFromContent(jid, {
         viewOnceMessage: {
            message: {
               interactiveMessage: {
      ...
      Miyan.relayMessage(jid, message["message"], {
         messageId: message.key.id
      })
   }
	
	Miyan.sendListImage = async (jid, sections, quoted, opts = {}) => {
      var image = await prepareWAMessageMedia({
         image: {
            url: opts && opts.image ? opts.image : ''
         }
	...
	Miyan.sendButtonMsg = async (jid, body = '', footer = '', title = '', media, buttons = [], quoted, options = {}) => {
		...
		await Miyan.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
	}
...
async function Serialize(Miyan, m, store) {
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.getQuotedObj = async () => {
				...
				return await Serialize(Miyan, q, store)
			}
			m.quoted.key = {
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf/i.test(mime)) {
					return Miyan.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return Miyan.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return Miyan.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
