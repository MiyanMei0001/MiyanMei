...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./MiyanMedia/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./MiyanMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./MiyanMedia/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./MiyanMedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./MiyanMedia/trash/${filename}.mp4`), unlink(`./MiyanMedia/trash/${filename}.gif`)])
    return buffer5
}
